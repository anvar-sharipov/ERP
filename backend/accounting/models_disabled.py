...
#         with transaction.atomic():  # ← обернуть
#             super().save(*args, **kwargs)
#             self._update_stock()
...
#         # 1. Если это расход или перемещение — сначала списываем с источника с блокировкой
...
#         # 2. Если это приход или перемещение — добавляем на склад-получатель
...
#     image = ProcessedImageField(upload_to=product_image_path, processors=[ResizeToFit(1600, 1600)], format="JPEG", options={"quality": 85, "optimize": True}, validators=[validate_image_size], verbose_name="Изображение")
#     thumbnail = ImageSpecField(source="image", processors=[ResizeToFit(200, 200)], format="JPEG", options={"quality": 80, "optimize": True})
...
#                 name="product_main_img_idx",
#             ),
#             models.Index(
#                 fields=["product", "sort_order"],
#                 name="product_sort_img_idx",
#             ),
#         ]
...
#     user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="employee", verbose_name="Пользователь системы")
...
#     # all_warehouses = models.BooleanField(default=False)
...
#             super().save(*args, **kwargs)  # сначала сохраняем чтобы получить pk
...
# #             prefix_map = {
# #                 'in': 'IN', 'out': 'OUT', 'move': 'MOV',
# #                 'return_in': 'RIN', 'return_out': 'ROUT',
# #             }
