...
const ChatPage = () => {
  ...
  return (
    // <div className="flex h-full rounded-lg overflow-hidden border border-slate-700/50">
    <div className="flex h-full rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700/50 bg-gray-50 dark:bg-slate-900">
      <div className="w-72 shrink-0 flex flex-col border-r border-slate-700/50 bg-slate-800/30">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50">
          <span className="text-sm font-semibold text-slate-200">Сообщения</span>
          <button onClick={() => setNewChatOpen(true)} className="w-7 h-7 rounded-lg bg-indigo-600 hover:bg-indigo-500 flex items-center justify-center transition-colors" title="Новая беседа">
      ...
      <div className="flex-1 flex flex-col bg-slate-900/20">
        {selectedConvId ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-indigo-400/40 gap-3">
            ...
            <span className="text-sm">Выберите беседу или создайте новую</span>
