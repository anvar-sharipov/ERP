...
import { FileText, Trash2, Check, CheckCheck } from "lucide-react";
...
export const MessageItem: React.FC<Props> = ({ message, onVisible, onDelete }) => {
  ...
  const isAudioAttachment = !!message.attachment_content_type?.startsWith("audio/");
  ...
  return (
    ...
    <motion.div
      ...
      className={`group flex items-end gap-2 px-3 py-1 ${isMe ? "flex-row-reverse" : "flex-row"}`}
    >
      ...
      <div className={`flex flex-col max-w-[70%] ${isMe ? "items-end" : "items-start"}`}>
        ...
        <div
          className={`
    relative px-3 py-2 rounded-2xl text-base leading-relaxed break-words shadow-sm
    ${
      message.is_deleted
        ...
        : isMe
          ? "bg-[#3390ec] dark:bg-[#2b5278] text-white rounded-br-sm"
          : "bg-white dark:bg-[#212d3b] text-gray-900 dark:text-slate-100 rounded-bl-sm border border-gray-200/70 dark:border-transparent"
    }
            ...
            <>
              {message.attachment_url &&
                (isImageAttachment ? (
                  ...
                  <VoiceMessagePlayer src={message.attachment_url} isMe={isMe} />
                ) : (
                  <a
                    ...
                    className={`flex items-center gap-2 rounded-lg px-2 py-1.5 mb-1 transition ${
                      isMe ? "bg-white/15 hover:bg-white/25" : "bg-gray-100 dark:bg-slate-800/60 hover:bg-gray-200 dark:hover:bg-slate-800"
                    }`}
                  >
          ...
          <div className={`flex items-center gap-1 mt-1 ${isMe ? "justify-end" : "justify-start"}`}>
            ...
            {isMe && !message.is_deleted && (
              <span className="inline-flex items-center">
                {isRead ? <CheckCheck className="w-3.5 h-3.5 text-white" /> : <Check className="w-3.5 h-3.5 text-white/60" />}
              </span>
            )}
          </div>
        </div>
