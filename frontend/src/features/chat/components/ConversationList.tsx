...
export const ConversationList: React.FC<Props> = ({ onSelect }) => {
  ...
  const handleSelect = (conv: any) => {
    ...
    console.log("other.photo_thumbnail:", JSON.stringify(other?.photo_thumbnail));
      ...
      <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-700/50">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Поиск..."
            // className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg bg-slate-700/60 text-slate-200 placeholder-slate-500 border border-slate-600/50 focus:outline-none focus:border-indigo-500 transition-colors"
      ...
      <div className="flex-1 overflow-y-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-indigo-400/50 gap-2 p-4">
            ...
            <span className="text-sm">{search ? "Ничего не найдено" : "Нет бесед"}</span>
          </div>
        ) : (
              ...
              >
                ...
                <div className="relative w-10 h-10 rounded-full shrink-0 overflow-hidden">
                  {(() => {
                    ...
                    return isValidMediaUrl(other?.photo_thumbnail) ? (
                      <img src={other.photo_thumbnail!} alt={name} className="w-full h-full object-cover" />
                    ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    {/* <span className={`text-sm font-medium truncate ${isActive ? "text-indigo-200" : "text-slate-200"}`}>{name}</span> */}
                  ...
                  <div className="flex items-center justify-between gap-1 mt-0.5">
                    <span className="text-xs text-slate-400 truncate">{lastMsg ? (lastMsg.sender_id === user?.id ? `Вы: ${lastMsg.text}` : lastMsg.text) : "Нет сообщений"}</span>
                    {hasUnread && (
