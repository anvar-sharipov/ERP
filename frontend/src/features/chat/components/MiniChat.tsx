...
export const MiniChat: React.FC = () => {
  const { isMiniChatOpen, toggleMiniChat } = useChat();
  ...
  return (
    <AnimatePresence>
      {isMiniChatOpen && (
        ...
        >
          {/* Шапка — drag handle */}
          <div onMouseDown={handleMouseDown} className="flex items-center justify-between px-3 py-2 bg-orange-700 dark:bg-orange-900 border-b border-slate-700/50 cursor-grab active:cursor-grabbing select-none shrink-0">
            ...
            <div className="flex items-center gap-1">
              {/* Назад к списку */}
              {selectedConvId && (
                <button onClick={() => setSelectedConvId(null)} className="px-2 py-0.5 text-xs text-slate-400 hover:text-white hover:bg-orange-800 rounded transition-colors">
                  ← Назад
                </button>
              )}
              {/* Свернуть */}
