        ...
        {!periodFrom || !periodTo ? (
          ...
          <OSVTable
            ...
            onRowDoubleClick={(row) => {
              // ✅ Drill-down — если у счёта настроено субконто (например "Контрагент"),
              // ведём в детализацию по субконто (SubcontoBreakdownPage) вместо сырого
              // Журнала проводок — так пользователь сразу видит "кто сколько должен",
              // а не плоский список проводок. Без субконто — как раньше, в Журнал.
              // Период/склад/филиал туда не передаём — SubcontoBreakdownPage читает их
              // сам, live из useDateStore() (см. CLAUDE.md про drill-down отчёты).
              if (row.has_subconto) {
                sessionStorage.setItem("selectedOsvAccountId", String(row.id));
                navigate(ROUTES.APP.ACCOUNTING_SUBCONTO_BREAKDOWN.replace(":accountId", String(row.id)));
                return;
              }
