...
interface FormData {
  ...
  content_type_filter: Record<string, any>;
}
...
const SubcontoTypesPage = () => {
  ...
  const openEdit = (item: SubcontoType) => {
    ...
    setForm({
      ...
      content_type_filter: item.content_type_filter || {},
    });
    setFormModal(true);
  };
      ...
      <Modal isOpen={formModal} onClose={() => setFormModal(false)} title={editing ? `${t("Edit")}: ${editing.name}` : t("AddSubcontoType")} closeOnOutsideClick={false}>
        <div className="space-y-4">
          ...
          <div className="space-y-3">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...
                    directory: null,
                    content_type_filter: {}, // ✅ заодно сбрасываем фильтр при смене модели
                  }))
                }
            ...
            {/* ✅ блок вынесен НАРУЖУ select-а, теперь это отдельный div после него */}
            {(contentTypes as any[]).find((ct) => ct.id === form.content_type)?.model === "counterparty" && (
              <div>
                ...
                <select
                  value={form.content_type_filter?.type__in?.[0] ?? ""}
                  onChange={(e) => {
                    const val = e.target.value;
                    setForm((p) => ({
                      ...p,
                      content_type_filter: val
                        ? { type__in: [val, "both"] } // ✅ клиент ИЛИ both / поставщик ИЛИ both
                        : {},
                    }));
                  }}
                  className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
