...
const PromoRows = ({ isPosted, promoItems, lineTotal, onRemove, columns, selectedKey, onSelectRow, onPreviewImage }: PromoRowsProps) => {
        ...
        >
          {columns.map((col) => (
            <td
              ...
              `}
            >
              <PromoCellContent col={col} row={row} lineTotal={lineTotal} onPreviewImage={onPreviewImage} />
            </td>
          ))}
...
const PromoCellContent = ({
  ...
  switch (col.key) {
    ...
    case "product":
      return (
        <span className="text-lg print:text-xl font-medium text-emerald-700 dark:text-emerald-300 italic">
    ...
    case "quantity":
      return <span className="block text-right text-base print:text-xl text-emerald-600 dark:text-emerald-400 font-medium">+{row.quantity}</span>;

    case "price":
      return <span className="block text-right font-mono text-base print:text-xl text-emerald-600 dark:text-emerald-400">0.00</span>;
    ...
    case "total":
      return <span className="block text-right font-mono text-base print:text-xl font-semibold text-emerald-600 dark:text-emerald-400">{fmt(lineTotal(row))}</span>;
