...
import { warehouseApi, priceTypeApi } from "../../../services/productApi";
...
import { newItemRow, lineTotal } from "./Vars";
...
const DocumentFormPage = () => {
  ...
  useEffect(() => {
    ...
    setSidebarContent(
      <div className="space-y-4">
        {/* Статус */}
        <div>
          <h4 className="font-bold text-indigo-300 mb-2">Статус</h4>
        ...
        <div className="pt-3 border-t border-indigo-900/30">
          <h4 className="font-bold text-indigo-300 mb-2">Итого</h4>
        ...
        {counterpartyBalance !== null && (
          <div className="pt-3 border-t border-indigo-900/30">
            <h4 className="font-bold text-indigo-300 mb-1">Сальдо контрагента</h4>
        ...
        {isEdit && doc && (
          <div className="pt-3 border-t border-indigo-900/30">
            ...
            <div className="text-xs text-indigo-200 space-y-1">
              <div>№ {doc.number}</div>
              <div>Создан: {new Date(doc.created_at).toLocaleDateString("ru-RU")}</div>
              {doc.posted_at && <div>Проведён: {new Date(doc.posted_at).toLocaleDateString("ru-RU")}</div>}
            </div>
          </div>
        )}
      </div>,
    );
  }, [setSidebarContent, items, header.discount_percent, isPosted, doc, isEdit, counterpartyBalance]);
  ...
  const postMutation = useMutation({
    ...
    onError: (err: any) => {
      if (err._handled) return;
      notify("error", err.response?.data?.detail || "Ошибка проведения");
      setPostConfirm(false);
    },
  });
  ...
  const unpostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["documents"] });
      setDocStatus("draft");
      notify("success", "Проведение отменено");
      setUnpostConfirm(false);
    },
    onError: (err: any) => {
      if (err._handled) return;
      notify("error", err.response?.data?.detail || "Ошибка отмены проведения");
      setUnpostConfirm(false);
    },
  });
  ...
  const removeItem = async (row: ItemRow) => {
    if (row.id && docId) {
      try {
        ...
        notify("error", "Ошибка удаления строки");
        return;
      }
    }
  ...
  const removeParticipant = async (p: ParticipantRow) => {
    if (p.id && docId) {
      try {
        ...
        notify("error", "Ошибка удаления участника");
        return;
      }
    }
  ...
  return (
    <RBACGuard isLoading={isEdit ? isLoading : false} error={isEdit ? error : null} canView={isEdit ? canPut : canPost} forbiddenText={t("ForbiddenText")}>
      ...
      <ConfirmModal
        ...
        message="После проведения документ нельзя редактировать. Продолжить?"
      ...
      <ConfirmModal
        ...
        message="Складские движения будут отменены. Продолжить?"
