// frontend/src/features/accounting/pages/Employees/PositionsPage.tsx
...
const PositionsPage = () => {
  ...
  const { canView, canPost, canPut, canDelete } = usePageAccess("position");

  const [highlightedId, setHighlightedId] = useState<number | null>(null);
  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<Position | null>(null);
  const [form, setForm] = useState<PositionForm>(EMPTY);
  ...
  const saveMutation = useMutation({
    mutationFn: (data: PositionForm) => positionApi.save(editing?.id ?? null, data),
  ...
  const filtered = useTableFilter(positions, {
    search: searchQuery,
  ...
  const toDelete = positions.find((x) => x.id === deleteId);
